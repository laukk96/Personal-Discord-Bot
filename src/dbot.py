...
_similar_questions_file = (
    path_dbot + "/char_questions/base_files/_similar_questions.json"
)
...
commander_in_chief = ["<rank: commander_in_chief>", 263853089419821056]  # puf
    ...
    if await isWhitelisted(ctx, commander_in_chief, "Shutdown the Raspberry Pi 4"):
        ...
        if countdown == None or countdown < 5:
            ...
            countdown_msg = await ctx.send(
                f":white_check_mark: Shutting down in {countdown} seconds..."
            )
            while countdown > 1:
                countdown -= 1
                time.sleep(1)
                await countdown_msg.edit(
                    content=f":white_check_mark: Shutting down in {countdown} seconds..."
                )
            ...
            await ctx.send(
                f":no_entry: Shutdowns can only be {maxShutdown} seconds or less."
            )
        # else:
...
async def reboot(ctx):
    if await isWhitelisted(ctx, commander_in_chief, "Reboot the Raspberry Pi 4"):
        reboot_msg = await ctx.send(
            ":white_check_mark: I'll be back :wink: \n`[ Rebooting Raspberry Pi ]`"
        )
...
async def isCorrectChannel(ctx, command):
    ...
    if command in the_guild and not the_guild[command] == ctx.channel.name:
        await ctx.send(
            f":no_entry: Incorrect channel for command `<{command}>`.", delete_after=3
        )
...
async def send_commands(ctx):
    ...
    for rank in all_commands:
        ...
        for cmd in all_commands[rank]:
            the_string = (
                the_string + f"{get_guild_prefix(ctx)}{cmd} {all_commands[rank][cmd]}\n"
            )
...
async def linkadd(ctx, param, your_link):
    if not f"{param}" in links_array[str(ctx.guild.id)]:
        ...
        await ctx.send(
            f":no_entry: Link acronym `{param}` already exists!", delete_after=5
        )
...
async def linkedit(ctx, param, your_link):
    if f"{param}" in links_array[str(ctx.guild.id)]:
        ...
        await ctx.send(
            f":no_entry: Link acronym `{param}` does not exist!", delete_after=5
        )
...
async def custom(ctx):
    if await isWhitelisted(ctx, custom_whitelist, can_send_fail=False):
        ...
        await ctx.send(
            f":raised_hand: :pensive: Your name does not start with K, "
            + ctx.author.name
        )
...
async def bind(ctx, command):
    if await isWhitelisted(
        ctx, admin_whitelist, perm="set a command's specified channel"
    ):
        if await canBind(ctx, command) == True:
            for rank in all_commands:
                if command in all_commands[rank]:
                    ...
                    await ctx.send(
                        f":white_check_mark: Set command `<{command}>` to channel {ctx.channel.mention}"
                    )
                    return
...
async def unbind(ctx, *, command):
    if await isWhitelisted(
        ctx, admin_whitelist, perm="unbind a command from a specified channel"
    ):
        previous_channel = unbind_command(ctx, command)
        await ctx.send(
            f":white_check_mark: Unbinded command `<{command}>` from channel `<{previous_channel}>`."
        )
...
async def kick(ctx, member: discord.Member, *, reason=None):
    if await isWhitelisted(ctx, admin_whitelist, perm="kick others"):
        if canKickOrBan(member) == True:
            ...
            await ctx.send(
                f":no_entry: Cannot kick a person of `{authority_whitelist[0]}`."
            )
...
async def ban(ctx, member: discord.Member, *, reason=None):
    if await isWhitelisted(ctx, admin_whitelist, perm="ban others"):
        if canKickOrBan(member) == True:
            ...
            await ctx.send(
                f":no_entry: Cannot ban a person of `{authority_whitelist[0]}`."
            )
...
work_help = {
    ...
    "fields": [
        # normal commands
        {"name": "`show`", "value": "Shows your current reminders are."},
        {
...
async def create_profile_embed(ctx, profile):
    ...
    for i in range(0, len(profile)):
        ...
        card_arr = {
            "name": f"`{card}` : {days} Days, {rd_hour}:{rd_min} {ampm}",
            "value": "_ _",
        }
...
async def check_work_profile(ctx, wr, uid, gid):
    if not uid in wr[gid]:
        ...
        await ctx.channel.send(
            f":white_check_mark: {ctx.author.mention} Your profile has been added to this guild's Work Reminder dictionary."
        )
        ...
        if arg[0] == "show" or arg[0] == "s":
            ...
            if len(arg) >= 2 and arg[1].isdigit():
                ...
                if str(arg[1]) in wr[gid]:
                    ...
                    await ctx.send(
                        f":white_check_mark: {ctx.author.mention} Profile for uid `{arg[1]}`: \n```{profile}```\nProfile in work stats: `{arg[1] in ws_dict[gid]}`"
                    )
                else:
                    # await ctx.send(f':no_entry: {ctx.author.mention} You have no cards in your dictionary.')
                    await ctx.send(
                        f":no_entry: {ctx.author.mention} User id `{arg[1]}` not found in Work Reminder dictionary."
                    )
            # CHECKING OWN DICTIONARY
            else:
                profile = wr[gid][uid]
                if len(profile) > 0:
                    check_embed = discord.Embed.from_dict(
                        await create_profile_embed(ctx, profile)
                    )
                    await ctx.send(embed=check_embed)
                else:
                    await ctx.send(
                        f":no_entry: {ctx.author.mention} You have no cards in your dictionary."
                    )
            ...
            if arg2 != None and arg3 != None and arg3.isdigit():
                ...
                if new_card_days >= 1 and new_card_days <= 30:
                    ...
                    write_json(wr, work_reminder_file)
                    await ctx.send(
                        f":white_check_mark: {ctx.author.mention} Added card `{arg2}` to Work Dictionary. \nThis card will recover after `{arg3}` days on `{rd}`."
                    )
                else:
                    await ctx.send(
                        f":no_entry: {ctx.author.mention} Invalid integer for `<number_of_days>`.",
                        delete_after=5,
                    )
            # check failed, invalid arguments
            else:
                await ctx.send(
                    f":no_entry: {ctx.author.mention} Your `<card>` or `<number_of_days>` cannot be invalid.",
                    delete_after=5,
                )
                time.sleep(5)
                await ctx.message.delete()
            ...
            await ctx.send(
                f":tools: {ctx.author.mention} Welcome to the **Work Reminder** command. Please type `.work help` for more information."
            )

        elif arg[0] == "setchannel" or arg[0] == "sc":
            if await isWhitelisted(
                ctx, commander_in_chief, "set the channel for Karuta Work Reminders."
            ):
                wr[gid]["setchannel"] = ctx.channel.id
                write_json(wr, work_reminder_file)
                await ctx.send(
                    f":white_check_mark: {ctx.author.mention} Set the channel to {ctx.channel.mention}"
                )
            ...
            if (
                arg[1] == "profile"
                or arg[1] == "p"
                and await isWhitelisted(
                    ctx, commander_in_chief, "clear a profile from the Work Dictionary."
                )
            ):
                ...
                if arg[2] == "all":
                    # on reaction here
                    msg = await ctx.send(
                        f":arrow_forward: {ctx.author.mention} Removing all profiles from `Work Dictionary`..."
                    )
                    to_pop = []
                    for uid, profile in wr[gid].items():
                    ...
                    if len(to_pop) > 0:
                        await msg.edit(
                            content=f":white_check_mark: {ctx.author.mention} All profiles have been removed from `Work Dictionary`."
                        )
                    else:
                        await msg.edit(
                            content=f":no_entry: {ctx.author.mention} No profiles in `Work Dictionary` to remove."
                        )
                # remove a specific profile
                    ...
                    if arg[2] in wr[gid]:
                        ...
                        await ctx.send(
                            f":white_check_mark: {ctx.author.mention} Successfully removed user profile `{arg[2]}` from Work Dictionary."
                        )
                    else:
                        await ctx.send(
                            f":no_entry: {ctx.author.mention} Failed to find user profile `{arg[2]}`",
                            delete_after=5,
                        )
                        time.sleep(5)
                        await ctx.message.delete()
            ...
            # clearing a card from dictionary
                ...
                for i in range(0, len(profile)):
                    ...
                    if the_card == target_card or the_card.find(target_card) != -1:
                        ...
                        write_json(wr, work_reminder_file)
                        await ctx.send(
                            f":white_check_mark: {ctx.author.mention} Successfully cleared `{the_card}` from Work Dictionary."
                        )
                        return
                await ctx.message.delete()
                await ctx.send(
                    f":no_entry: {ctx.author.mention} Did not find card `{target_card}` in your Work Dictionary.",
                    delete_after=5,
                )
            else:
                await ctx.message.delete()
                await ctx.send(
                    f":no_entry: {ctx.author.mention} Invalid usage of command `<clear>`.",
                    delete_after=5,
                )
    elif arg[0] == "initiate" or arg[0] == "i":
        if (
            await isWhitelisted(
                ctx, commander_in_chief, "initiate a guild for Karuta Work Reminder."
            )
            and not gid in wr
        ):
            wr[gid] = {}
            ...
            await ctx.send(
                ":white_check_mark: Initiated this guild for `Karuta Work Reminder`."
            )
            await ctx.send(
                f":white_check_mark: Set the channel to {ctx.channel.mention}"
            )
        else:
            await ctx.send(
                f":no_entry: {ctx.author.mention} This guild is already initialized!"
            )
    else:
        await ctx.message.delete()
        await ctx.send(
            f":no_entry: {ctx.author.id} You must initiate this guild for work first.",
            delete_after=5,
        )
        time.sleep(3)
        await ctx.message.delete()
...
frame_help = {
    ...
    "fields": [
        # normal commands
        {"name": "`.frame <bit>`", "value": "Shows what frames use a bit."},
        {"name": "`.frame print`", "value": "Prints all the frames"},
        {
...
async def frame(ctx, *, args=""):
    ...
    if checkingBit != False:
        ...
        await ctx.send(
            f":white_check_mark: {ctx.author.mention} Frames that use the `{checkingBit}` bit:\n{all_frames_string}"
        )
        ...
        await ctx.send(
            f":no_entry: {ctx.author.mention} Invalid use of `.frame` command. See `.frame help` for more information."
        )
...
async def frameadd(ctx, *, args=""):
    if await isWhitelisted(
        ctx, commander_in_chief, "add a frame to the `frame_info.json` file."
    ):
        args = args.split(" ")
        ...
        if (
            len(fname) > 0
            and bits[0] in all_bits["bits"]
            and bits[1] in all_bits["bits"]
        ):
            frame_info = open_json(frame_info_file)
            new_frame = {"frame": fname, "bits": bits}
            if index != None:
            ...
            await ctx.send(
                f":white_check_mark: {ctx.author.mention} Successfully sent the frame `{fname}` with the bits `{bits}` to `frame_info.json`."
            )
        else:
            await ctx.send(
                f":no_entry: {ctx.author.mention} Invalid usage of `.frameadd` command. `(Invalid Parameters or Bits)`"
            )
...
async def nodeadd(ctx, *, args=""):
    if await isWhitelisted(
        ctx, commander_in_chief, "add a node to the `bits.json` file."
    ):
        args = args.split(" ")
        all_bits = open_json(bits_file)
        if not args[0] in all_bits["bits"]:
            ...
            await ctx.send(
                f":white_check_mark: {ctx.author.mention} Successfully added bit node `{args[0]}` to `bits.json` file."
            )
        else:
            await ctx.send(
                f":no_entry: {ctx.author.mention} The bit node `{args[0]}` is already in the `bits.json` file!"
            )
    ...
    if arg1 == "set":
        if await isWhitelisted(ctx, commander_in_chief, "set the maximum dice roll."):
            if arg2.isdigit():
                ...
                await ctx.send(
                    f":white_check_mark: Set the max dice roll to `{max_dice_roll}`."
                )
            else:
                await ctx.send(f":no_entry: Invalid integer.")
    elif arg1 == None:
        await ctx.send(
            f":game_die: Your roll was: `{random.randint(1, max_dice_roll)}`! {ctx.author.mention}"
        )
    else:
        ...
        if desc.find(inj_str) != -1:
            ...
            while len(desc) > 1:
                ...
                await message.channel.send(
                    f":white_check_mark: {user.mention} `{card}` has been added to reminder dictionary. It will recover in `{days}` days."
                )

            if ws_dict[gid][uid]["no_injury_streak"] > 0:
                await message.channel.send(
                    f':octagonal_sign: {user.mention} You have broken your streak of working `{ws_dict[gid][uid]["no_injury_streak"]}` time*(s)* without an injury!'
                )
                ws_dict[gid][uid]["no_injury_streak"] = 0
                write_json(ws_dict, work_stats_file)
            write_json(wr, work_reminder_file)
            ...
            celebrate_index = random.randint(
                0, len(work_success_emotes) - 1
            )  # includes both ends
            ws_dict[gid][uid]["no_injury_streak"] += 1
            write_json(ws_dict, work_stats_file)
            await message.channel.send(
                f':white_check_mark: {user.mention} You have successfully worked without injury. \n{work_success_emotes[celebrate_index]} Your injury free streak is: `{ws_dict[gid][uid]["no_injury_streak"]}`!'
            )
        else:
            await message.channel.send(
                f":ballot_box_with_check: {user.mention} You decided to cancel your work."
            )
...
async def timeloop(ctx, arg1=None):
    ...
    if await isWhitelisted(ctx, commander_in_chief, "set the time loop seconds."):
        if arg1 == None:
            await ctx.send(
                f":clock1: {ctx.author.mention} The time loop is running every `{timeloop_seconds}` seconds."
            )
            ...
            time_loop.restart()
            await ctx.send(
                f":white_check_mark: {ctx.author.mention} Changed the time loop interval to `{timeloop_seconds}` seconds."
            )
        else:
            await ctx.send(
                f":no_entry: {ctx.author.mention} Invalid integer for seconds."
            )
...
async def send_reminder(setchannel_id, uid, card):
    ...
    now = datetime.now()
    await channel.send(
        f":alarm_clock: {user.mention} Your card `{card}` has recovered! "
    )
            ...
            if uid != "last_updated" and uid != "setchannel":
                ...
                if len(profile) > 0:
                    ...
                    last_updated = datetime(
                        last_updated[0], last_updated[1], last_updated[2]
                    )
                    ...
                    while i < len(profile):
                        ...
                        if profile[i]["days"] <= 0 and timedelta_secs > 0:
                            ...
                            await send_reminder(
                                wr[server_id]["setchannel"], uid, profile[i].get("card")
                            )
                            profile.pop(i)
                        else:
                            i = i + 1
    ...
    for server_id in wr:
        ...
        if (
            ...
            if timedelta_compare.days >= 1:
                await send_remind_check(
                    wr[server_id]["setchannel"], last_updated_str, today
                )
...
egg_help_embed = {
    ...
    "fields": [
        ...
        {"name": "`.egg clear`", "value": "Clears your egg reminder dictionary."},
...
async def egg(ctx, *, param=""):
    ...
    if not uid in egg_dict:
        egg_dict[uid] = []
        await ctx.send(
            f":white_check_mark: {ctx.author.mention} Your profile was added to the egg dictionary."
        )
                    ...
                    for i in range(1, len(param)):
                        if param[i].isdigit() and not param[i] in egg_dict[uid]:
                            if int(param[i]) < 1 or int(param[i]) > 20:
                                await ctx.send(
                                    f":no_entry: {ctx.author.mention} One of your eggs was not a valid number between `(1-20)`!"
                                )
                            else:
                                egg_dict[uid].append(param[i])
                        else:
                            await ctx.send(
                                f":no_entry: {ctx.author.mention} One of your eggs was not a number. Try again."
                            )
                            return
                    write_json(egg_dict, egg_dict_file)
                    await ctx.send(
                        f":white_check_mark: {ctx.author.mention} You have set your eggs to: `{egg_dict[uid]}`."
                    )
                else:
                    await ctx.send(
                        f":no_entry: {ctx.author.mention} You used the `set` command without adding any eggs!"
                    )
                print(f"{ctx.author.name}'s EGG DICT: \n{egg_dict[uid]}")
            else:
                await ctx.send(
                    f":no_entry: {ctx.author.mention} You added too many eggs! `[Maximum of 5]`."
                )
        elif param[0] == "clear" or param[0] == "c":
            egg_dict[uid] = []
            write_json(egg_dict, egg_dict_file)
            await ctx.send(
                f":white_check_mark: {ctx.author.mention} Egg dictionary has been cleared."
            )
        elif (
            param[0] == "print"
            or param[0] == "p"
            and await isWhitelisted(ctx, commander_in_chief, "print the egg dictionary")
        ):
            await ctx.send(f"```\n{egg_dict}\n```")
        else:
            await ctx.send(
                f":no_entry: {ctx.author.mention} Invalid usage of command `egg`."
            )
...
async def eggs(ctx):
    ...
    if uid in egg_dict:
        if len(egg_dict[uid]) > 0:
            await ctx.send(
                f":egg: {ctx.author.mention} You are currently looking for eggs `{egg_dict[uid]}`."
            )
        else:
            await ctx.send(
                f":no_entry: {ctx.author.mention} You do not have any eggs in your egg dictionary! Add some using the `set` command."
            )
    else:
        await ctx.send(
...
async def egg_claimed(message):
    if message.author.id == 646937666251915264:
        if "you placed **Springtide Egg" in message.content:
            ...
            egg_dict = open_json(egg_dict_file)
            print(
                "\t> IS EGG {egg} INSIDE {message.mentions[0]}'s dict?: {egg in egg_dict[uid]}"
            )
            if uid in egg_dict:
                # if egg in egg_dict[str(uid)]:
                for i in range(0, len(egg_dict[uid])):
                    print(
                        f"checking {message.mentions[0]}: '{egg_dict[uid][i]}', is equal to '{egg}'?: {int(egg_dict[uid][i]) == int(egg)}"
                    )
                    if int(egg_dict[uid][i]) == egg:
                        egg_dict[uid].pop(i)
                        write_json(egg_dict, egg_dict_file)
                        await message.channel.send(
                            f":white_check_mark: {message.mentions[0].mention} `Egg #{egg}` has been removed from your egg dictionary.\n Remaining: `{egg_dict[uid]}`."
                        )
                        return
                ...
                if not uid in egg_dict:
                    await message.channel.send(
                        f":no_entry: User `{message.mentions[0]}` does not have an egg dictionary.",
                        delete_after=10,
                    )

                elif not egg in egg_dict[uid]:
                    await message.channel.send(
                        f":no_entry: User `{message.mentions[0]}` does not have `Egg #{egg}` in their egg dictionary.",
                        delete_after=10,
                    )
            ...
            for uid in egg_dict:
                ...
                if len(egg_dict[uid]) > 0:
                    for wanted_egg in egg_dict[uid]:
                        print(
                            f"> check {uid}: wanted: {wanted_egg}, checking: {new_egg}, same? = {int(wanted_egg == new_egg)}"
                        )
                        if int(wanted_egg) == new_egg:
                            ...
                            if len(egg_dict[uid]) == 1:
                                await reaction.message.channel.send(
                                    f":egg: <:pogu:819461856875773972> :warning: The Last Egg :warning: that you're missing `Egg #{new_egg}` has spawned! {person.mention}"
                                )
                                break
                            else:
                                await reaction.message.channel.send(
                                    f":egg: An egg that you're missing `Egg #{new_egg}` has spawned! {person.mention}"
                                )
                                break
...
def remove_expired_manual_question(manual_question_id):
    check_entry_bool, entry = find_message_id_in_manual_questions_array(
        manual_question_id
    )
    if check_entry_bool != False:
        ...
        print(
            f" Attempted to remove manual question of id: ({manual_question_id}) from array. Unsuccessful, deleted already."
        )
...
async def on_reaction_add(reaction, user):
    ...
    if (
        reaction.message.author.id == 731644582164430870
        and user.id != 731644582164430870
    ):
        correct_reaction_bool, reactioned_manual_question = (
            find_message_id_in_manual_questions_array(reaction.message.id)
        )
        ...
        if (
            correct_reaction_bool != False
            and reactioned_manual_question["user_id"] == user.id
        ):
...
def return_answer_group_and_index(simq, question, answers):
    ...
    for agi in range(len(simq["answer_group"])):
        ...
        if bool_is_ag_matching == True:
            ...
            target_ag_index = agi
            print(
                f"> return_answer_group_and_index(): \nFound Question/answer pair: \n\t> Question: {question} \n\tAnswers: {answers}"
            )
            return found_target_ag, target_ag_index
...
def return_sorted_simq(simq, question, question_index, answers):
    ...
    found_target_ag, target_ag_index = return_answer_group_and_index(
        simq, question, answers
    )
    ...
    if await isWhitelisted(ctx, admin_whitelist, perm="can edit character files"):
        ...
        for filename in os.listdir(_pathdir):
            ...
            if not filename == "_question_storage.json" and filename.endswith(".json"):
                ...
                write_json(char_file, char_file_path)
                await ctx.send(
                    f"{ctx.author.mention} Added `{num}` question slots to all `character.json` files."
                )
    ...
    if args == None or args == "help":
        ...
        if args[0].isdigit() and len(args) >= 2:
            ...
            if 0 <= args[0] and args[0] < len_questions:
                ...
                char_file = find_file(
                    f"{char}.json", "/home/pi/discordbot/char_questions"
                )
                ...
                if char_file != None:
                    ...
                    if char_correct_status == True:
                        char_correct_status = (
                            f":white_check_mark: **{char_correct_status}**"
                        )
                    else:
                    ...
                    if char_answers != None:
                        if char_file["correct?"][args[0]] == True:
                            ...
                            for i in range(len(char_file["answers"][args[0]])):
                                ...
                                char_answers += (
                                    f"{qstor['answers'][args[0]][answer_index]}"
                                )
                    ...
                    await ctx.message.delete()
                    await ctx.send(
                        f":no_entry: {ctx.author.mention} Character file with the name `{char}` has not been found.",
                        delete_after=5,
                    )
            else:
                await ctx.send(
                    f":no_entry: {ctx.author.mention} Question #{args[0]} does not exist!"
                )
        # Character info
        elif args[0] == "info":
            await ctx.send(
                f"{ctx.author.mention} Character info command not implemented yet."
            )
        # Similar questions info
            ...
            while args_index < len(args):
                ...
                if args_index == len(args) and adding_to_alias == False:
                    await ctx.send(
                        f":no_entry: {ctx.author.mention} Incorrect Alias formatting. ***(Include quotations)***"
                    )
                    return None
            ...
            await ctx.send(
                f":white_check_mark: {ctx.author.mention} Character `{char}` has been added to `koibito_aliases.json` with Alias: `{alias}`"
            )
            ...
            if len(args) > 1 and args[1].isdigit():
                ...
                if 0 <= group_index and group_index < len(
                    simq["similar_question_indexes"]
                ):
                    # comment 3 : write info for char
                    mstr = f"> :white_check_mark: {ctx.author.mention} Similar Group #**{group_index}**"
                    mstr += f"\n> Questions: **"
                    max_question_index = len(
                        simq["similar_question_indexes"][group_index]
                    )
                    ...
                    random_similar_question_index = random.randint(
                        0, len(simq["similar_question_indexes"][group_index]) - 1
                    )
                    random_showcase_index = simq["similar_question_indexes"][
                        group_index
                    ][random_similar_question_index]
                    ...
                    await ctx.send(
                        f":no_entry: {ctx.author.mention} Similar group of index {group_index} is invalid!"
                    )
            else:
                simq = open_json(_similar_questions_file)
                await ctx.send(
        ...
        # Send date guide image
            ...
            await ctx.message.delete()
            await ctx.send(
                "https://media.discordapp.net/attachments/809242709607841864/868903659629600848/date-guide.png",
                delete_after=120,
            )
    ...
    if len(message.embeds) > 0:
        ...
        if emb["title"] == "Visit Character":
            if "footer" in emb:
                ...
                if (
                    emb["footer"]["text"].find(
                        "Choose the response most likely to impress "
                    )
                    != -1
                ):
                    ...
                    # text_unanswered = "Choose the response most likely to impress "
                    char = footer[
                        footer.find(text_unanswered)
                        + len(text_unanswered) : len(footer)
                    ]
                    ...
                    uid = int(uid)
                    question_index = (
                        None  # index of question inside _question_storage.json
                    )
                    ...
                    if not question in qstor["questions"]:
                        ...
                        simq = open_json(_similar_questions_file)
                        simq = return_sorted_simq(
                            simq, question, question_index, answers
                        )
                        write_json(simq, _similar_questions_file)

                        await message.channel.send(
                            f":white_check_mark: {client.get_user(uid).mention} \n> Added Question **#{len(qstor['questions']) - 1}**: *`{question}`*."
                        )
                    else:
                        question_index = qstor["questions"].index(question)
                    ...
                    if char_file == None:
                        ...
                        new_char_path = (
                            f"/home/pi/discordbot/char_questions/{char}.json"
                        )
                        create_new_char_file(char_file, new_char_path)
                        await message.channel.send(
                            f"{client.get_user(uid).mention} Created new file `{char}.json`."
                        )
                    ...
                    if char_file["correct?"][question_index] != True:
                        ...
                        manual_question_text = (
                            f":grey_question: {client.get_user(uid).mention}"
                        )
                        simq = open_json(_similar_questions_file)

                        # Setup for Similar Question Algorithm
                        found_target_ag, target_ag_index = (
                            return_answer_group_and_index(simq, question, answers)
                        )
                        ...
                        for i in range(
                            len(simq["similar_question_indexes"][target_ag_index])
                        ):
                            simq_index = simq["similar_question_indexes"][
                                target_ag_index
                            ][i]
                            ...
                            if char_file["answers"][simq_index] != None:
                                print(
                                    f"\t\t> SIMQ answer: {qstor['answers'][simq_index]}\n"
                                )
                                ...
                                simq_question = highlight_koibito_keywords(
                                    simq_question
                                )

                                # manual_question_text += f"\n> :pencil: #**{simq_index}** `{simq_question}`"
                                if char_file["correct?"][simq_index] == True:
                                    correct_answer_index = char_file["answers"][
                                        simq_index
                                    ]
                                    simq_answer = qstor["answers"][simq_index][
                                        correct_answer_index
                                    ]

                                    # If out of the already displayed correct answers, the duplicate isn't there
                                    if (
                                        not simq_answer[1 : len(simq_answer)]
                                        in past_correct_answers
                                    ):
                                        # Add correct question text
                                        manual_question_text += f"\n> :pencil: #**{simq_index}** `{simq_question}`"
                                        past_correct_answers.append(
                                            simq_answer[1 : len(simq_answer)]
                                        )
                                        manual_question_text += (
                        ...
                        manual_question = await message.channel.send(
                            manual_question_text
                        )
                        ...
                        print(
                            f"> Added manual question of id: ({manual_question.id}) into manual_questions_array."
                        )
                        ...
                        correct_answer = qstor["answers"][question_index][
                            correct_answer_index
                        ]
                        ...
                        await message.channel.send(
                            f":white_check_mark::grey_question: {client.get_user(uid).mention} \nPrevious correct answer: {correct_answer}"
                        )

            # After a question is answered, no footer found
            elif (
                not "footer" in emb
                and emb["description"].find("Your Affection Rating has ") != -1
            ):
                ...
                uid = int(uid)
                answered_question_entry = (
                    find_question_message_id_in_manual_questions_array(before.id)
                )
                ...
                if answered_question_entry != False:
                    ...
                    question_index = answered_question_entry[
                        "question_index"
                    ]  # index of question inside _question_storage.json
                    ...
                    if emb["description"].find("increased by") != -1:
                        ...
                        if (
                            ...
                            actual_answer_str = actual_answer_str[
                                1 : len(actual_answer_str)
                            ]

                            answer = get_correct_answer_index(
                                qstor, actual_answer_str, question_index
                            )
                    ...
                    # If answered incorrectly, store answer in array of incorrect answers
                        ...
                        if answer != None:
                            ...
                            if (
                                isinstance(char_file["answers"][question_index], list)
                                == True
                            ):
                            ...
                            # If there was no entry / If the answer was correct, but upon double-check it is proven false
                                ...
                                char_file["answers"][question_index] == None
                                or char_file["answers"][question_index].isdigit()
                            ):
                            ...
                            answer = qstor["answers"][question_index][answer]
                            await message.channel.send(
                                f":no_entry: {client.get_user(uid).mention} #**{question_index}** \nCharacter: `{char}` Answer: `{answer}`"
                            )
...
async def on_ready():
    ...
    init_time_loop()
    await client.change_presence(activity=discord.Game(f"Ara Ara~ "))
